...
export default function ProviderBreakdown({ entries, compact = false }: ProviderBreakdownProps) {
  ...
  const allEntries = [...ordered, ...extras.map((e) => ({ label: e.label, count: e.count }))];
      ...
      <div className="flex flex-wrap gap-2">
        {allEntries.map((entry) => {
          ...
          return (
            <div
              key={entry.label}
              className={`inline-flex items-center gap-2 rounded-xl border border-slate-200/80 px-3 py-2 ${colors.bg}`}
            >
