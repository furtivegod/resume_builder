...
import type { DailyBidPoint } from "@/lib/dashboard-stats";

interface DailyBidChartProps {
  points: DailyBidPoint[];
  compact?: boolean;
}

export default function DailyBidChart({ points, compact = false }: DailyBidChartProps) {
  ...
  if (points.length === 0) {
    return (
      <div className={compact ? "chart-panel-compact" : "chart-panel"}>
        <p className="mb-2 text-sm font-semibold text-slate-900 dark:text-slate-50">Daily bids</p>
        <p className="rounded-lg border border-dashed border-slate-200 dark:border-slate-600/60 bg-slate-50/80 dark:bg-slate-800/90 px-3 py-6 text-center text-xs text-slate-500 dark:text-slate-300">
          Select a start and end date to see daily bid counts.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto pb-1">
        ...
        >
          {points.map((point, index) => {
            const height =
              point.count > 0 && maxCount > 0
                ? Math.max(8, (point.count / maxCount) * (chartHeight - 4))
                : 0;
            ...
            return (
              <div
                ...
                title={`${formatShortDate(point.date)}: ${point.count} bid${point.count === 1 ? "" : "s"}`}
              >
                {point.count > 0 && (
                ...
                <div
                  ...
                  style={{ height: point.count > 0 ? height : 4 }}
                />
